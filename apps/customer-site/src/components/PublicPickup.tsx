...
export const PublicPickup: React.FC<PublicPickupProps> = ({ config, language, onOrderSuccess, setRoute }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="min-h-[max(884px,100dvh)] overflow-x-hidden bg-[#F2F2F2] font-sans text-[#1f1a23]"
    ...
    >
      ...
      <main className="mx-auto max-w-[1440px] px-4 pb-36 pt-24 md:px-16">
        <div className="mb-12 text-center md:text-start">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-[#F2F2F2] px-3 py-1 font-mono text-[12px] font-semibold uppercase leading-4 tracking-[0.1em] text-[#090A0D]">
            <CalendarDays className="size-3.5" aria-hidden="true" />
            {t('خطوة الحجز: الموعد والاستلام', 'BOOKING STEP: PICKUP SCHEDULE')}
          </div>
          <h2 className="font-display text-[32px] font-bold leading-[40px] text-[#1f1a23] md:text-[48px] md:leading-[56px] md:tracking-[-0.02em]">
            {t('متى نمر عليك؟', 'When shall we visit?')}
          </h2>
          <p className="mx-auto mt-2 max-w-2xl text-[18px] font-normal leading-[28px] text-[#4c4354] md:mx-0">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  !done && !active && 'text-[#4c4354] hover:bg-white/70',
                )}
              >
                <span className={cn('grid size-7 shrink-0 place-items-center rounded-full text-xs font-black', active ? 'bg-white text-primary' : done ? 'bg-primary text-white' : 'bg-[#F2F2F2] text-[#4c4354]')}>
          ...
          <div className="mb-8 rounded-[24px] border border-primary/20 bg-primary/5 p-5 text-start">
            <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-[#4c4354]">
        ...
        <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
          <div className="space-y-8 lg:col-span-8">
            {currentStep === 0 ? <GlassCard>
              <div className="mb-6 flex items-center justify-between gap-4">
                ...
                <span className="font-mono text-[12px] font-semibold uppercase leading-4 tracking-[0.1em] text-[#4c4354]">
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        active ? 'border-primary bg-[#F2F2F2]/30 ring-1 ring-primary' : 'border-[#cfc2d6]',
                      )}
                    >
                      <span>
                        <span className="block text-base font-bold text-[#1f1a23]">{area.name}</span>
                        <span className="mt-1 block text-xs font-semibold text-[#4c4354]">
            ...
            {currentStep === 1 ? <GlassCard>
                  ...
                  return (
                    ...
                    >
                      <span className="min-w-0">
                        <span className="block text-base font-bold text-[#1f1a23]">{service.name}</span>
                        <span className="mt-1 block text-sm leading-6 text-[#4c4354]">{service.desc}</span>
              ...
              <div className="mt-5 rounded-2xl border border-primary/10 bg-primary/5 p-4 text-sm leading-6 text-[#4c4354]">
            ...
            {currentStep === 3 ? <GlassCard>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <label className="grid gap-2">
                  <span className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.08em] text-[#4c4354]">
                  ...
                  <input
                    ...
                    className="min-h-12 rounded-2xl border border-[#cfc2d6] bg-white px-4 text-[#1f1a23] outline-none transition placeholder:text-[#7e7385] focus:border-primary focus:ring-2 focus:ring-primary/20"
              ...
              <label className="mt-5 grid gap-2">
                ...
                <textarea
                  ...
                  className="min-h-24 resize-none rounded-2xl border border-[#cfc2d6] bg-white px-4 py-3 text-[#1f1a23] outline-none transition placeholder:text-[#7e7385] focus:border-primary focus:ring-2 focus:ring-primary/20"
            ...
            {currentStep === 2 ? <GlassCard>
              <div className="mb-6 flex items-center justify-between gap-4">
                ...
                <span className="font-mono text-[12px] font-semibold uppercase leading-4 tracking-[0.1em] text-[#4c4354]">{t('توقيت الإمارات', 'UAE TIME (GMT+4)')}</span>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        active ? 'border-primary ring-2 ring-primary/20' : 'border-[#cfc2d6]',
                      )}
                    >
                      <span className={cn('mb-2 font-mono text-[12px] font-semibold uppercase leading-4 tracking-[0.1em]', active ? 'text-primary' : 'text-[#4c4354]')}>
                        {date.label}
                      </span>
                      <span className="text-2xl font-bold text-[#1f1a23]">{date.day}</span>
                      <span className="text-xs font-medium text-[#4c4354]">{date.month}</span>
                    </button>
                  );
                })}
              </div>
                  ...
                  return (
                    ...
                    >
                      <span className="flex flex-col items-start">
                        <span className="font-mono text-sm font-semibold uppercase text-[#4c4354] group-hover:text-primary">{slot.label}</span>
                        <span className="text-lg font-bold text-[#1f1a23]">{slot.time}</span>
                      </span>
                      <CheckCircle2 className={cn('size-5 text-primary transition-opacity', active ? 'opacity-100' : 'opacity-0')} />
                    </button>
                  );
            ...
            {currentStep === 2 ? <GlassCard>
              ...
              <div className="space-y-4">
                <label className={cn('relative flex cursor-pointer items-center rounded-2xl border p-6 transition hover:bg-[#F2F2F2]/20', delivery === 'standard' ? 'border-primary bg-[#F2F2F2]/10' : 'border-[#cfc2d6] bg-white')}>
                  <input
                  ...
                  <div className="ms-4 flex-1">
                    <div className="flex items-center justify-between gap-4">
                      <span className="text-lg font-bold text-[#1f1a23]">{t('توصيل عادي', 'Standard Delivery')}</span>
                      <span className="font-medium text-[#4c4354]">{t('مشمول', 'Included')}</span>
                    </div>
                    <p className="mt-1 text-sm text-[#4c4354]">{t('إرجاع خلال 48 ساعة مع تنظيف وفحص جودة.', 'Returned within 48 hours. Thorough cleaning and quality check included.')}</p>
                  </div>
                </label>

                <label className={cn('relative flex cursor-pointer items-center rounded-2xl border p-6 transition hover:border-primary', delivery === 'express' ? 'border-primary bg-[#F2F2F2]/10' : 'border-[#cfc2d6] bg-white')}>
                  <input
                  ...
                  <div className="ms-4 flex-1">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-lg font-bold text-[#1f1a23]">{t('توصيل سريع', 'Express Delivery')}</span>
                        <span className="rounded bg-[#00f1fe] px-2 py-0.5 text-[10px] font-bold uppercase text-[#006a70]">{t('أولوية', 'Priority')}</span>
                      </div>
                      <span className="font-bold text-primary">+AED 50</span>
                    </div>
                    <p className="mt-1 text-sm text-[#4c4354]">{t('إرجاع خلال 24 ساعة للمواعيد العاجلة.', 'Returned within 24 hours. Perfect for urgent events and tight schedules.')}</p>
                  </div>
                </label>
              </div>
            </GlassCard> : null}
            ...
            {currentStep === 2 ? <GlassCard>
              ...
              <div className="relative">
                <textarea
                  className="min-h-32 w-full resize-none rounded-2xl border border-[#cfc2d6] bg-white px-6 py-4 text-[#1f1a23] outline-none transition placeholder:text-[#7e7385] focus:border-primary focus:ring-2 focus:ring-primary/20"
                  placeholder={t('مثال: اتصل قبل الوصول بعشر دقائق...', 'Example: Gate code is 1234, please leave bags with the concierge...')}
                  rows={4}
                  value={notes}
                  onChange={(event) => setNotes(event.target.value)}
                />
                <div className="absolute bottom-4 right-4 flex gap-2 text-[#cfc2d6]">
            ...
            {currentStep === 4 ? <GlassCard>
              ...
              <p className="mt-5 text-sm leading-7 text-[#4c4354]">
          ...
          <aside className="space-y-6 lg:col-span-4 lg:sticky lg:top-28">
            <GlassCard>
              ...
              <div className="mb-6 space-y-4 border-b border-[#cfc2d6] pb-6">
                <div className="flex justify-between text-[#4c4354]">
                  <span>{t('الخدمات', 'Services')} (8)</span>
                  <span className="font-medium text-[#1f1a23]">AED {baseServiceEstimate.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-[#4c4354]">
                  <span>{t('رسوم السرعة', 'Express Surcharge')}</span>
                  <span className="font-medium text-[#1f1a23]">AED {delivery === 'express' ? '50.00' : '0.00'}</span>
                </div>
                <div className="flex justify-between text-[#4c4354]">
                  <span>{t('رسوم التوصيل', 'Delivery Fee')}</span>
                  <span className="font-medium text-[#1f1a23]">AED {deliveryFee.toFixed(2)}</span>
                </div>
              </div>
              <div className="mb-8 flex items-end justify-between">
                <div>
                  <p className="font-mono text-[12px] font-semibold uppercase leading-4 tracking-[0.1em] text-[#7e7385]">{t('الإجمالي التقريبي', 'ESTIMATED TOTAL')}</p>
                  <p className="text-3xl font-extrabold text-primary">AED {total.toFixed(2)}</p>
                </div>
                <span className="text-end text-xs text-[#4c4354]">{t('شامل الضريبة', 'VAT included')}</span>
              </div>
              {validationError ? (
              ...
              <div className="flex gap-4 rounded-2xl bg-[#fbf0fe] p-4">
                <div className="grid size-12 flex-shrink-0 place-items-center rounded-xl border border-[#cfc2d6] bg-white shadow-sm">
                  <ShieldCheck className="size-5 text-primary" aria-hidden="true" />
                </div>
                <div>
                  <p className="text-sm font-bold">{t('ضمان العناية', 'Premium Assurance')}</p>
                  <p className="text-xs text-[#4c4354]">{t('يتم التعامل مع القطع بعناية وفحص جودة قبل التسليم.', 'Your items are handled with care and quality checked before delivery.')}</p>
                </div>
              </div>
            </GlassCard>
      ...
      <nav className="fixed bottom-0 left-0 z-50 w-full border-t border-white/40 bg-white/60 px-4 pb-8 pt-4 backdrop-blur-2xl">
        <div className="mx-auto flex w-full max-w-[1440px] flex-col items-center justify-between gap-4 md:flex-row">
          <div className="hidden items-center gap-6 md:flex">
            ...
            <div className="h-6 w-px bg-[#cfc2d6]" />
            <div className="flex items-center gap-2">
          ...
          <div className="flex w-full gap-4 md:w-auto">
            ...
            <button
              ...
              className="flex h-12 flex-[2] items-center justify-center gap-3 rounded-full bg-primary px-12 font-bold text-white shadow-lg shadow-primary/30 transition hover:bg-[#8C2370] active:scale-95 disabled:opacity-60 md:h-14 md:w-72"
            >
              {isSubmitting
...
const InfoMini = ({ label, value }: { label: string; value: string }) => (
  <div className="min-w-0">
    <p className="text-[11px] font-black uppercase tracking-[0.08em] text-[#7e7385]">{label}</p>
    <p className="mt-1 truncate text-sm font-bold text-[#1f1a23]">{value}</p>
  </div>
);
