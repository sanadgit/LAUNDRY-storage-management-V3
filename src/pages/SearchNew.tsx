...
import Grid2D from '../components/Grid2DNew';
...
export default function SearchPageNew() {
  ...
  // New loading state for picked actions inside the inspector panel
  ...
  useEffect(() => {
    ...
    if (!hasSearchInput) {
      // In SearchNew, left panel remains open on desktop, but we handle mobile snap states
  ...
  // Left Search panel remains visible on desktop/tablet viewport under both search and input modes
  ...
  // Hooking up current store definition for Slot Inspector
  const currentStore = useMemo(
    () => stores.find((s) => s.store_name === selectedStore) || stores[0],
    [stores, selectedStore]
  );

  // Hooking up selected cell items for Slot Inspector
  const selectedSlotItems = useMemo(() => {
    if (!selectedGridCell) return [];
    return blankets.filter(
      (b) =>
        b.store === selectedGridCell.store &&
        b.row === selectedGridCell.row &&
        b.column === selectedGridCell.column &&
        b.status === 'stored'
    );
  }, [blankets, selectedGridCell]);

  // Handler for Picking items inside the Slot Inspector list
  const handlePicked = async (blanket: Blanket) => {
    ...
    try {
      ...
      setPickError(err?.message || 'Failed to mark as picked.');
    } finally {
      setPickingId(null);
    }
  };
  ...
  return (
    <div className="warehouse-search-ui h-full w-full min-w-0 flex flex-col bg-slate-900 text-white overflow-hidden">
      ...
      <div className="warehouse-main-content flex-1 relative flex min-w-0 overflow-hidden">
        {/* Left Sidebar: Results & Guided Retrieval / Slot Inspector */}
        <div className={cn(
          ...
          {retrievalMode && storedMatches.length > 0 ? (
            ...
            <div className="warehouse-search-panel-scroll p-4 sm:p-6 flex flex-col h-full min-h-0 overflow-y-auto pb-[max(1rem,env(safe-area-inset-bottom))]">
              {/* Detailed Slot Inspector when in Input Mode (no search query) */}
              {!hasSearchInput ? (
                <div className="flex flex-col gap-6 h-full">
                  {/* Title / Section Header */}
                  <div className="flex items-center gap-3 text-emerald-400">
                    <Package size={24} />
                    <h2 className="text-[clamp(1.15rem,1.9vw,1.7rem)] font-black uppercase tracking-tight">Slot Inspector</h2>
                  </div>

                  {/* Active Store Stats Widget */}
                  {currentStore && (
                    <div className="rounded-3xl border border-[#203A5C] bg-[#142238]/60 p-5 space-y-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-[10px] font-black uppercase tracking-widest text-[#9FB0C7]">ACTIVE SECTION</div>
                          <div className="text-2xl font-black text-white">{currentStore.store_name}</div>
                        </div>
                        <span className="px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full bg-blue-500/10 text-[#2F7DFF] border border-[#203A5C]">
                          {currentStore.store_type || 'Grid'}
                        </span>
                      </div>

                      <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[#203A5C]/40">
                        <div>
                          <div className="text-[9px] font-bold text-[#64748B] uppercase">Layout</div>
                          <div className="text-sm font-black text-white">{currentStore.rows}R × {currentStore.columns}C</div>
                        </div>
                        <div>
                          <div className="text-[9px] font-bold text-[#64748B] uppercase">Capacity Limit</div>
                          <div className="text-sm font-black text-white">{(currentStore as any).slot_capacity || 1} Per Cell</div>
                        </div>
                      </div>
                    </div>
                  )}

                  {/* Selected Cell Info and Items */}
                  <div className="flex-1 flex flex-col min-h-0 bg-[#071426]/70 border border-[#203A5C] rounded-3xl p-5">
                    {selectedGridCell ? (
                      <div className="flex flex-col h-full min-h-0">
                        <div className="flex items-center justify-between pb-3 border-b border-[#203A5C]/60 mb-4">
                          <div>
                            <div className="text-[9px] font-black uppercase tracking-widest text-[#9FB0C7]">SELECTED COORDINATES</div>
                            <div className="text-base font-black text-[#3DFFD0]">
                              Row {selectedGridCell.row} · Column {selectedGridCell.column}
                            </div>
                          </div>
                          <span className={cn(
                            "px-2.5 py-1 text-[10px] font-black uppercase tracking-wider rounded-lg border",
                            selectedSlotItems.length >= ((currentStore as any).slot_capacity || 1)
                              ? "bg-[#3A1028]/80 text-[#FF9BC7] border-[#B01F63]"
                              : selectedSlotItems.length > 0
                              ? "bg-[#216BFF]/10 text-[#2F7DFF] border-[#203A5C]"
                              : "bg-slate-800/50 text-slate-400 border-slate-700"
                          )}>
                            {selectedSlotItems.length} / {(currentStore as any).slot_capacity || 1} Stored
                          </span>
                        </div>

                        {/* Items list inside the cell */}
                        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                          {selectedSlotItems.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-center p-6 text-[#64748B]">
                              <Crosshair size={32} className="opacity-40 mb-2 animate-pulse" />
                              <div className="text-xs font-bold uppercase tracking-wider">Empty Slot</div>
                              <p className="text-[11px] mt-1">Select this cell in the grid to input items.</p>
                            </div>
                          ) : (
                            selectedSlotItems.map((item) => (
                              <div
                                key={item.id}
                                className="rounded-2xl border border-[#203A5C] bg-[#142238]/40 p-3.5 flex items-center justify-between gap-3 hover:bg-[#142238]/60 transition-colors"
                              >
                                <div className="min-w-0">
                                  <div className="text-sm font-black text-white truncate">#{item.blanket_number}</div>
                                  <div className="text-[9px] text-[#9FB0C7] font-semibold">
                                    Stored {new Date(item.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                  </div>
                                </div>
                                <button
                                  type="button"
                                  onClick={() => handlePicked(item)}
                                  disabled={pickingId === item.id || !canPick}
                                  className={cn(
                                    "px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 transition-all shadow-md active:scale-95",
                                    pickingId === item.id || !canPick
                                      ? "bg-[#142238] text-[#64748B] border border-slate-700 cursor-not-allowed"
                                      : "bg-[#10A978] hover:bg-[#10A978]/90 text-white border border-[#3DFFD0]/30 shadow-emerald-950/40"
                                  )}
                                >
                                  {pickingId === item.id ? (
                      ...
                      <div className="h-full flex flex-col items-center justify-center text-center p-6 text-[#64748B]">
                        <MapIcon size={36} className="opacity-40 mb-2 animate-bounce" />
                        <div className="text-xs font-bold uppercase tracking-widest">No Cell Selected</div>
                        <p className="text-[11px] mt-1">Tap a slot in the grid map to inspect or load blankets.</p>
                      </div>
                    )}
                  </div>

                  {/* Operation Quick Guide */}
                  <div className="rounded-2xl bg-[#0D1B2E] border border-[#203A5C] p-4 flex gap-3 items-start">
                    <AlertCircle size={18} className="text-[#2F7DFF] shrink-0 mt-0.5" />
                    <div className="text-[11px] leading-relaxed text-[#9FB0C7]">
                      <span className="font-bold text-white uppercase block mb-1">LOGISTICS QUICK TIP</span>
                      To search or locate a specific order, type the blanket number in the top bar. The grid will automatically highlight target coordinates in <span className="text-[#3DFFD0] font-bold">green</span>.
                    </div>
                  </div>
                </div>
              ) : exactMatches.length === 0 ? (
        ...
        {/* Center/Right Layout: Viewport + Static Bottom Bar for Desktop/Tablet */}
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-slate-950">
          <div className="warehouse-viewport flex-1 min-w-0 relative overflow-hidden" style={mobileViewportBottomInsetStyle}>
            ...
            {/* Mobile Store Selector Overlay (Only visible on mobile) */}
            {isMobileViewport && showInputStoreSelector && (
              <div
                className={cn(
                  "absolute bottom-2 left-1 right-1 bg-slate-900/80 backdrop-blur-md p-1.5 rounded-2xl border border-slate-700 shadow-2xl z-10 overflow-x-auto no-scrollbar transition-all",
                  hideMobileStoreSelector && "hidden"
                )}
          ...
          {/* Desktop/Tablet Static Bottom Bar */}
          {!isMobileViewport && showInputStoreSelector && (
            <div className="bottom-store-bar flex-none border-t border-slate-800 bg-slate-900/70 p-3 flex justify-center items-center">
              <StoreControlBar
                stores={stores}
                selectedStore={selectedStore}
                lockedStores={lockedStores}
